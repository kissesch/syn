...
describe("sqlite backend: blob staging", () => {
	it("stages a blob and increments storage used bytes", async () => {
		const { unitOfWork, blobStore, healthStore } =
			await createSqliteCoordinator();

		await stage(unitOfWork, "blob-1", 1_000, 100, 200);
	...
	it("rejects a blob larger than the configured max file size", async () => {
		const { unitOfWork } = await createSqliteCoordinator("vault-1", {
			storageLimitBytes: 1_000_000_000,
			maxFileSizeBytes: 10,
			versionHistoryRetentionDays: 1,
		});

		await expect(stage(unitOfWork, "blob-1", 11, 100, 200)).rejects.toThrow(
	...
	it("rejects a blob that would exceed the vault storage quota", async () => {
		...
		await expect(stage(unitOfWork, "blob-1", 2_000, 100, 200)).rejects.toThrow(
			SyncCoordinatorApplicationError,
		);
	});
	...
	it("does not mutate storage_used_bytes when a stage is rejected mid-transaction", async () => {
		const { unitOfWork, blobStore, healthStore } =
			await createSqliteCoordinator("vault-1", {
				storageLimitBytes: 1_000,
				maxFileSizeBytes: 10_000_000,
				versionHistoryRetentionDays: 1,
			});

		await stage(unitOfWork, "blob-a", 500, 100, 200);
		expect(healthStore.readStorageStatus().storageUsedBytes).toBe(500);

		await expect(stage(unitOfWork, "blob-b", 900, 100, 200)).rejects.toThrow(
			SyncCoordinatorApplicationError,
		);
	...
	it("rejects re-staging a blob that is already live", async () => {
		const { unitOfWork, blobStore, mutationService } =
			await createSqliteCoordinator();
		await stage(unitOfWork, "blob-1", 100, 1_000, 2_000);
		...
		await expect(
			stage(unitOfWork, "blob-1", 100, 3_000, 4_000),
		).rejects.toThrow(SyncCoordinatorApplicationError);
	});

	it("collects a staged-but-never-committed blob once its grace period passes", async () => {
		const { unitOfWork, blobStore, blobGcStore } =
			await createSqliteCoordinator();
		await stage(unitOfWork, "blob-1", 100, 1_000, 1_500);

		unitOfWork.stores.versions.expireEntryVersions(2_000);
		const ready = blobGcStore.listCollectibleBlobs(2_000, 10);
		expect(ready.map((row) => row.blob_id)).toContain("blob-1");

		deleteCollectible(unitOfWork, ["blob-1"], 2_000);
	...
	it("deletes a collectible batch in one statement and skips referenced blobs", async () => {
		const { unitOfWork, blobStore, handle, healthStore } =
			await createSqliteCoordinator();
		await stage(unitOfWork, "blob-a", 100, 1_000, 1_500);
		await stage(unitOfWork, "blob-b", 50, 1_000, 1_500);
		await stage(unitOfWork, "blob-referenced", 25, 1_000, 1_500);
		handle.exec(
			`
			INSERT INTO entries (
	...
	it("deletes more collectible blobs than the durable object parameter limit allows", async () => {
		...
		for (const blobId of blobIds) {
			await stage(unitOfWork, blobId, 1, 1_000, 1_500);
		}

		expect(
			deleteCollectible(unitOfWork, blobIds, 2_000)
				.map((blob) => blob.blob_id)
				.sort(),
		).toEqual([...blobIds].sort());
		expect(blobIds.every((blobId) => blobStore.readBlob(blobId) === null)).toBe(
			true,
		);
	...
	it("deletes an unreferenced staged blob and reports a missing row", async () => {
		...
		expect(deleteUnreferencedStagedBlob(unitOfWork, "blob-1", 1_500)).toBe(
			"deleted",
		);
		expect(blobStore.readBlob("blob-1")).toBeNull();
		expect(healthStore.readStorageStatus().storageUsedBytes).toBe(0);
	...
	it("marks an unreferenced live blob pending-delete through the GC store", async () => {
		const { unitOfWork, blobStore, handle } = await createSqliteCoordinator();
		await stage(unitOfWork, "blob-1", 100, 1_000, 2_000);
		handle.exec(
		...
		markBlobPendingDeleteIfUnpinned(unitOfWork, "blob-1", 3_000);
	...
	it("keeps a blob collectible only after retained history expires", async () => {
		const { unitOfWork, blobGcStore, handle } = await createSqliteCoordinator();
		await stage(unitOfWork, "blob-history", 100, 1_000, 1_500);
		handle.exec(
		...
		unitOfWork.stores.versions.expireEntryVersions(3_000);
		expect(
			blobGcStore.readCollectibleBlob("blob-history", 3_000),
		).toMatchObject({
	...
	it("returns the earliest future GC deadline", async () => {
		const { unitOfWork, blobGcStore } = await createSqliteCoordinator();
		await stage(unitOfWork, "blob-deadline", 100, 1_000, 5_000);

		expect(earliestGcDeadline(blobGcStore.readGcDeadlines(2_000), 2_000)).toBe(
			5_000,
		);
		expect(earliestGcDeadline(blobGcStore.readGcDeadlines(5_000), 5_000)).toBe(
			5_000,
		);
		deleteCollectible(unitOfWork, ["blob-deadline"], 5_000);
		expect(
			earliestGcDeadline(blobGcStore.readGcDeadlines(5_000), 5_000),
		).toBeNull();
	});
	...
	it("keeps due GC work scheduled after the first deletion batch", async () => {
		...
		for (let i = 0; i < GC_BATCH_SIZE + 1; i += 1) {
			await stage(unitOfWork, `blob-${i}`, 1, 1_000, 1_500);
		}
	...
	it("does not keep scheduling a due staged blob that is referenced", async () => {
		const { unitOfWork, blobGcStore, handle } = await createSqliteCoordinator();
		await stage(unitOfWork, "blob-referenced-due", 100, 1_000, 1_500);
		handle.exec(
			`
			INSERT INTO entries (
				entry_id, revision, blob_id, encrypted_metadata, deleted,
		...
		expect(
			earliestGcDeadline(blobGcStore.readGcDeadlines(2_000), 2_000),
		).toBeNull();
	});
});

async function stage(
	unitOfWork: CoordinatorUnitOfWork,
	blobId: string,
	sizeBytes: number,
	now: number,
	deleteAfter: number,
) {
	return stageBlobForTest(unitOfWork, blobId, sizeBytes, now, deleteAfter);
}
function deleteUnreferencedStagedBlob(
	...
	return unitOfWork.run((stores) => deleteStaged(stores, blobId, now));
}
...
function deleteCollectible(
	...
	return unitOfWork.run((stores) =>
		deleteCollectibleBlobRecords(stores, ids, now),
	);
}
